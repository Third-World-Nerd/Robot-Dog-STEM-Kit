...
from config import INTERPOLATION_STEPS_X, INTERPOLATION_STEPS_Y
...
class WalkingMechanism:
    ...
    def bezier_path(self):
        ...
        scale_x = self.step_lengthX / (np.max(control_points[:, 0]) - np.min(control_points[:, 0]))
        scale_y = self.step_lengthY / (np.max(control_points[:, 1]) - np.min(control_points[:, 1]))
...
wm = [WalkingMechanism(HIP_KNEE_LENGTH, KNEE_FOOT_LENGTH, FOOT_POSITIONS_WALK[0], STEP_LENGTH_X, STEP_LENGTH_Y, 0, INTERPOLATION_STEPS_X, INTERPOLATION_STEPS_Y),
      ...
      WalkingMechanism(HIP_KNEE_LENGTH, KNEE_FOOT_LENGTH, FOOT_POSITIONS_WALK[2], STEP_LENGTH_X, STEP_LENGTH_Y, 1, INTERPOLATION_STEPS_X, INTERPOLATION_STEPS_Y),   #1
      WalkingMechanism(HIP_KNEE_LENGTH, KNEE_FOOT_LENGTH, FOOT_POSITIONS_WALK[3], STEP_LENGTH_X, STEP_LENGTH_Y, 0, INTERPOLATION_STEPS_X, INTERPOLATION_STEPS_Y),
    ]
